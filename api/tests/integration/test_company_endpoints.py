...
def test_update_company(client, company_payload):
    # Create first
    res_create = client.post("/company", json=company_payload)
    company = res_create.get_json()
    guid = company["external_guid"]

    # PATCH
    update_data = {"name": fake.company()}
    res = client.patch(f"/company/{guid}", json=update_data)
    assert res.status_code == 200
    data = res.get_json()
    assert data["name"] == update_data["name"]
    assert data["external_guid"] == guid
...
def test_delete_company(client, company_payload):
    ...
    # DELETE
    res = client.delete(f"/company/{guid}")
    assert res.status_code == 204

    # Verify deletion
    res = client.get(f"/company/{guid}")
...
def test_company_lifecycle(client):
    ...
    # CREATE
    res_create = client.post("/company", json=payload)
    ...
    # UPDATE
    update_data = {"name": fake.company()}
    res_update = client.patch(f"/company/{guid}", json=update_data)
    assert res_update.status_code == 200
    assert res_update.get_json()["name"] == update_data["name"]

    # GET
    res_get = client.get(f"/company/{guid}")
    assert res_get.status_code == 200
    assert res_get.get_json()["name"] == update_data["name"]

    # DELETE
    res_delete = client.delete(f"/company/{guid}")
    assert res_delete.status_code == 204

    # Confirm deletion
    res_get_deleted = client.get(f"/company/{guid}")
